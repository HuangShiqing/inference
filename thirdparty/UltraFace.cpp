...
using namespace std;
    ...
    UltraFace::UltraFace(const std::string &mnn_path,int input_width, int input_length, int num_thread_,
                     float score_threshold_, float iou_threshold_, int topk_) {
    image_w = input_width;
    image_h = input_length;
...
int UltraFace::detect(unsigned char* buffer, std::vector<FaceInfo> &face_list) {
    // if (raw_image.empty()) {
    ...
    std::shared_ptr<MNN::CV::ImageProcess> pretreat(
            MNN::CV::ImageProcess::create(MNN::CV::RGB, MNN::CV::RGB, mean_vals, 3,
                                          norm_vals, 3));
    pretreat->convert(buffer, in_w, in_h, in_w*3, input_tensor);
...
void UltraFace::faceinfo2det(std::vector<FaceInfo> face_info, detection* dets)
{
    for(unsigned int i=0;i<face_info.size();i++)
    {
        dets[i].bbox.h= (face_info[i].y2 - face_info[i].y1)/image_h;
        dets[i].bbox.w= (face_info[i].x2 - face_info[i].x1)/image_w;
        dets[i].bbox.x= (face_info[i].x2 + face_info[i].x1)/2/image_w;
        dets[i].bbox.y= (face_info[i].y2 + face_info[i].y1)/2/image_h;
        dets[i].max_prob = face_info[i].score;
        dets[i].max_class = 0;
    }
}